...
const getMeaningParts = (item) => {
  const parts = [
    item.noun_meaning && item.noun_meaning.trim() && { tag: "n", meaning: item.noun_meaning.trim() },
    item.verb_meaning && item.verb_meaning.trim() && { tag: "v", meaning: item.verb_meaning.trim() },
    item.adj_meaning && item.adj_meaning.trim() && { tag: "adj", meaning: item.adj_meaning.trim() },
  ].filter(Boolean);
  if (parts.length > 0) return parts;
  if (item.meaning && item.meaning.trim()) return [{ tag: null, meaning: item.meaning.trim() }];
...
const generateVocabQuestions = (selectedData, fullData, level) => {
  // Gộp toàn bộ nghĩa (đã tách riêng) của cả kho từ, dùng để rút đáp án nhiễu
  const meaningPool = fullData.flatMap((d) =>
    getMeaningParts(d)
      .filter((p) => p.meaning)
  ...
  return selectedData.flatMap((item) => {
    ...
    return meaningParts.map((part) => {
      ...
      const displayMeaning = part.meaning ? part.meaning.replace(/^\s*\([^)]*\)\s*[-–]\s*/, '').trim() : "";
      const tagLabel = part.tag ? `(${part.tag}) ` : "";
      let questionObj = { ...item, type: qType, meaning: `${tagLabel}${displayMeaning}`, meaningTag: part.tag };

     if (qType === "en_to_vn" || qType === "listening") {
      const displayAnswer = displayMeaning;
      const correctLen = displayAnswer.length || 1;
      const similarPool = meaningPool.filter((p) => {
        if (p.word === item.word || !p.meaning || p.meaning === displayAnswer) return false;
        const ratio = p.meaning.length / correctLen;
      ...
      {currentQ.type === "flashcard" && (
        <div style={{ animation: "popIn 0.3s ease-out", display: "flex", flexDirection: "column", alignItems: "center" }}>
            {flashcardPhase === "learn" ? (
                <>
                    <h2 style={{ fontSize: "18px", color: "#2c3e50", marginBottom: "8px", fontWeight: "600" }}>Lật thẻ để học từ 🎴</h2>
                    {currentQ.topic && (
                      <span style={{ display: "inline-block", fontSize: "12px", fontWeight: "bold", color: "#8e24aa", backgroundColor: "#f3e5f5", padding: "3px 12px", borderRadius: "20px", border: "1px solid #ce93d8", marginBottom: "12px" }}>
                        🏷️ {currentQ.topic}
                      </span>
                    )}
...
function GrammarNotesPanel({ notes, currentUser, onRefreshNotes }) {
  ...
  const [importTargetNote, setImportTargetNote] = useState(null);
  ...
  const summarizeNote = async (note) => {
    ...
    setSummaries(prev => ({ ...prev, [note.id]: { status: "loading", text: "", userEdited: false } }));
...
Hãy tóm tắt:`;
      ...
      // Lưu vào state
      setSummaries(prev => ({ ...prev, [note.id]: { status: "done", text, userEdited: false } }));
      ...
      if (currentUser) {
        ...
        if (docSnap.exists()) {
          ...
          await updateDoc(userDocRef, {
            "grammar.notesSummaries": { ...currentSummaries, [note.id]: { status: "done", text, userEdited: false, savedAt: Date.now() } }
          });
        }
      }
      ...
      setSummaries(prev => ({ ...prev, [note.id]: { status: "error", text: "Lỗi: " + err.message, userEdited: false } }));
    }
  };

// Hàm trích xuất danh sách title từ markdown cũ
const extractTopicsFromMarkdown = (markdownText) => {
  if (!markdownText) return [];
  const matches = markdownText.match(/^## (.+)$/gm);
  if (!matches) return [];
  return matches.map(m => m.replace(/^## /, '').trim());
};

// Hàm nhập tóm tắt từ ngoài vào (hỗ trợ ghép phần mới)
const handleImportSummary = async (note, formattedText, newJsonData = null) => {
  if (!formattedText.trim()) {
    alert("Vui lòng nhập nội dung tóm tắt!");
    return;
  }

  const oldSummary = summaries[note.id];
  let finalText = formattedText;
  let isMerged = false;

  // Nếu đã có tóm tắt cũ và có dữ liệu JSON mới
  if (oldSummary?.text && newJsonData?.topics) {
    const oldTopics = extractTopicsFromMarkdown(oldSummary.text);
    const newTopics = newJsonData.topics.map(t => t.title);
    
    // Tìm các chủ đề mới chưa có trong tóm tắt cũ
    const newTopicsOnly = newTopics.filter(t => !oldTopics.includes(t));
    
    if (newTopicsOnly.length > 0 && newTopicsOnly.length < newTopics.length) {
      // Chỉ thêm phần mới, giữ nguyên phần cũ
      let newSectionsText = "";
      for (const topic of newJsonData.topics) {
        if (newTopicsOnly.includes(topic.title)) {
          newSectionsText += `## ${topic.title}\n\n`;
          if (topic.formulas?.length) newSectionsText += `### 🔹 Công thức / Cấu trúc\n${topic.formulas.map(f => `- ${f}`).join('\n')}\n\n`;
          if (topic.usages?.length) newSectionsText += `### 🔹 Cách dùng\n${topic.usages.map(u => `- ${u}`).join('\n')}\n\n`;
          if (topic.examples?.length) newSectionsText += `### 🔹 Ví dụ\n${topic.examples.map(e => `- ${e}`).join('\n')}\n\n`;
          if (topic.toeic_tips?.length) newSectionsText += `### 🔹 Lưu ý khi làm bài thi TOEIC\n${topic.toeic_tips.map(t => `- ${t}`).join('\n')}\n\n`;
        }
      }
      
      finalText = oldSummary.text + `\n\n--- PHẦN MỚI (${new Date().toLocaleDateString('vi-VN')}) ---\n\n` + newSectionsText;
      isMerged = true;
    }
  }

  setSummaries(prev => ({ ...prev, [note.id]: { status: "done", text: finalText, userEdited: true, merged: isMerged } }));
  ...
  if (currentUser) {
    ...
    if (docSnap.exists()) {
      ...
      const currentSummaries = currentData?.grammar?.notesSummaries || {};
      await updateDoc(userDocRef, {
        "grammar.notesSummaries": { ...currentSummaries, [note.id]: { status: "done", text: finalText, userEdited: true, merged: isMerged, savedAt: Date.now() } }
      });
    }
  }
  
  setShowImportModal(false);
  setImportTargetNote(null);
  setImportText("");
  setExpandedId(note.id);
  playSound(isMerged ? "combo_3" : "finish");
  if (isMerged) {
    alert(`✅ Đã thêm ${newJsonData?.topics.filter(t => !extractTopicsFromMarkdown(oldSummary?.text || "").includes(t.title)).length} chủ đề mới vào tóm tắt!`);
  }
};

  // Hàm xóa tóm tắt
  const handleDeleteSummary = async (noteId) => {
    if (!confirm("Bạn có chắc muốn xóa tóm tắt này? (Có thể tóm tắt lại bằng AI sau)")) return;
    ...
    if (currentUser) {
      ...
      if (docSnap.exists()) {
        ...
        delete currentSummaries[noteId];
        await updateDoc(userDocRef, {
  ...
  const formatSummary = (text, isUserEdited) => {
    ...
    return (
      <div>
        ...
        {isUserEdited && (
          <div style={{ marginTop: "8px", fontSize: "10px", color: "#4CAF50", fontStyle: "italic", borderTop: "1px solid #c8e6c9", paddingTop: "6px" }}>
  ...
  return (
    <div style={{ background: "#f0f7ff", borderBottom: "2px solid #bbdefb", padding: "10px 14px", flexShrink: 0, maxHeight: "320px", overflowY: "auto", scrollbarWidth: "none" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
        <div style={{ fontSize: "12px", fontWeight: "900", color: "#1565c0", letterSpacing: "0.5px" }}>📄 FILE NGỮ PHÁP ĐÃ UPLOAD ({notes.length})</div>
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
        {notes.map(note => {
...
{/* Modal nhập tóm tắt từ ngoài */}
{showImportModal && importTargetNote && (
  <div onClick={() => setShowImportModal(false)} style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.6)", zIndex: 1300, display: "flex", justifyContent: "center", alignItems: "center", padding: "20px", boxSizing: "border-box" }}>
    <div onClick={(e) => e.stopPropagation()} style={{ backgroundColor: "white", width: "100%", maxWidth: "650px", borderRadius: "16px", padding: "20px", animation: "popIn 0.3s", boxShadow: "0 10px 30px rgba(0,0,0,0.3)", cursor: "default", maxHeight: "90vh", overflowY: "auto" }}>
      <h3 style={{ margin: "0 0 8px 0", color: "#1565c0" }}>📝 Nhập tóm tắt cho "{importTargetNote.filename}"</h3>
      
      {/* Thông báo version */}
      <div style={{ marginBottom: "15px", padding: "8px 12px", backgroundColor: "#fff8e1", borderRadius: "8px", borderLeft: "4px solid #FF9800", fontSize: "12px" }}>
        📄 File hiện tại: v{importTargetNote.version || 1} | 
        📅 Upload: {new Date(importTargetNote.uploadedAt).toLocaleDateString('vi-VN')}
        {importTargetNote.lastSummaryUpdate && (
          <span> | 🕐 Tóm tắt lần cuối: {new Date(importTargetNote.lastSummaryUpdate).toLocaleDateString('vi-VN')}</span>
        )}
      </div>
      
      {/* BƯỚC 1: COPY PROMPT JSON */}
      <div style={{ marginBottom: "20px", padding: "12px", backgroundColor: "#e3f2fd", borderRadius: "10px", border: "1px solid #90caf9" }}>
        <div style={{ fontWeight: "bold", color: "#1565c0", marginBottom: "8px", fontSize: "13px" }}>
          📋 Bước 1: Copy prompt bên dưới → Gửi vào ChatGPT/Gemini/Claude cùng với FILE WORD MỚI NHẤT của bạn
        </div>
        <textarea
  readOnly
...
LƯU Ý: Phần "formulas", "usages", "examples", "toeic_tips" là các mảng, mỗi phần tử là một chuỗi text có thể chứa bullet points hoặc định dạng markdown đơn giản để khi hiển thị được đẹp mắt.`
  ...
  style={{ marginTop: "8px", padding: "6px 12px", backgroundColor: "#2196F3", color: "white", border: "none", borderRadius: "6px", fontSize: "12px", cursor: "pointer", fontWeight: "bold" }}
>
  📋 Copy Prompt (JSON)
</button>
      </div>
      ...
      <div style={{ marginBottom: "15px" }}>
        <div style={{ fontWeight: "bold", color: "#FF9800", marginBottom: "8px", fontSize: "13px" }}>
          📌 Bước 2: Paste JSON từ AI vào đây (tóm tắt TOÀN BỘ file mới nhất)
        </div>
        <textarea
          value={importText}
  ...
  "topics": [
    {
      ...
      "formulas": ["If + S + V(quá khứ), S + would/could + V(nguyên thể)", "Were + S + to V, S + would + V"],
      "usages": ["Diễn tả giả định không có thật ở hiện tại", "Dùng để đưa ra lời khuyên"],
      "examples": ["If I had a million dollars, I would buy a car.", "Were she to study harder, she would pass."],
      "toeic_tips": ["Động từ 'to be' luôn chia 'were'", "Đáp án thường chứa 'would'"]
    },
    {
      "title": "Câu Giả Định",
      "formulas": ["S + suggest/ recommend/ insist + that + S + (should) + V(nguyên thể)"],
      "usages": ["Diễn tả yêu cầu, đề nghị, sự cần thiết"],
      "examples": ["I suggest that he (should) study harder."],
      "toeic_tips": ["Động từ ở mệnh đề that luôn là V(nguyên thể)", "Should có thể được lược bỏ"]
    }
  ]
}`}
          rows={14}
      ...
      {/* Hiển thị so sánh nếu đã có summary cũ */}
      {(() => {
        const oldSummary = summaries[importTargetNote.id];
        if (oldSummary?.text && importText.trim()) {
          try {
            const newData = JSON.parse(importText.trim());
            if (newData.topics) {
              const oldTopics = extractTopicsFromMarkdown(oldSummary.text);
              const newTopics = newData.topics.map(t => t.title);
              const existingTopics = oldTopics.filter(t => newTopics.includes(t));
              const newTopicsOnly = newTopics.filter(t => !oldTopics.includes(t));
              
              if (existingTopics.length > 0 && newTopicsOnly.length === 0) {
                return (
                  <div style={{ marginBottom: "15px", padding: "10px", backgroundColor: "#fff3e0", borderRadius: "8px", borderLeft: "4px solid #FF9800" }}>
                    <span style={{ fontWeight: "bold", color: "#e65100" }}>⚠️ Cảnh báo:</span> Có {existingTopics.length} chủ đề đã tồn tại trong tóm tắt cũ. 
                    Bạn có muốn <strong>GHI ĐÈ</strong> toàn bộ không?
                  </div>
                );
              }
              if (newTopicsOnly.length > 0) {
                return (
                  <div style={{ marginBottom: "15px", padding: "10px", backgroundColor: "#e8f5e9", borderRadius: "8px", borderLeft: "4px solid #4CAF50" }}>
                    <span style={{ fontWeight: "bold", color: "#2e7d32" }}>✅ Phát hiện {newTopicsOnly.length} chủ đề MỚI:</span>
                    <ul style={{ margin: "5px 0 0 20px", fontSize: "12px" }}>
                      {newTopicsOnly.map(t => <li key={t}>{t}</li>)}
                    </ul>
                    <span style={{ fontSize: "11px", color: "#666" }}>Hệ thống sẽ CHỈ THÊM phần mới, giữ nguyên phần cũ.</span>
                  </div>
                );
              }
            }
            ...
            onClick={() => {
              try {
                ...
                if (!parsed.topics || !Array.isArray(parsed.topics) || parsed.topics.length === 0) {
                  alert("⚠️ JSON phải có trường 'topics' là mảng không rỗng!");
                  return;
                }
                for (let i = 0; i < parsed.topics.length; i++) {
                  const t = parsed.topics[i];
                  if (!t.title || !Array.isArray(t.formulas) || !Array.isArray(t.usages) || 
                      !Array.isArray(t.examples) || !Array.isArray(t.toeic_tips)) {
                    alert(`⚠️ Chủ đề ${i+1} thiếu trường bắt buộc!`);
                    return;
                  }
                }
                alert(`✅ JSON hợp lệ! ${parsed.topics.length} chủ đề ngữ pháp.`);
              } catch (e) {
                alert("❌ JSON không hợp lệ: " + e.message);
              }
            }}
            style={{ padding: "6px 12px", backgroundColor: "#9C27B0", color: "white", border: "none", borderRadius: "6px", fontSize: "12px", cursor: "pointer", fontWeight: "bold" }}
          >
            🔍 Kiểm tra JSON ({importText.trim().split('"title"').length - 1} chủ đề)
          </button>
        </div>
      )}
        ...
        <button 
          onClick={() => {
            try {
              ...
              // Chuyển JSON thành markdown đẹp
              let formattedText = "";
              for (const topic of parsed.topics) {
                formattedText += `## ${topic.title}\n\n`;
                if (topic.formulas?.length) formattedText += `### 🔹 Công thức / Cấu trúc\n${topic.formulas.map(f => `- ${f}`).join('\n')}\n\n`;
                if (topic.usages?.length) formattedText += `### 🔹 Cách dùng\n${topic.usages.map(u => `- ${u}`).join('\n')}\n\n`;
                if (topic.examples?.length) formattedText += `### 🔹 Ví dụ\n${topic.examples.map(e => `- ${e}`).join('\n')}\n\n`;
                if (topic.toeic_tips?.length) formattedText += `### 🔹 Lưu ý khi làm bài thi TOEIC\n${topic.toeic_tips.map(t => `- ${t}`).join('\n')}\n\n`;
              }
              
              handleImportSummary(importTargetNote, formattedText, parsed);
            } catch (e) {
              alert("❌ JSON không hợp lệ: " + e.message);
            }
          }} 
          disabled={!importText.trim()}
          style={{ flex: 1, padding: "12px", backgroundColor: importText.trim() ? "#4CAF50" : "#ccc", color: "white", border: "none", borderRadius: "8px", fontWeight: "bold", cursor: importText.trim() ? "pointer" : "not-allowed" }}
        >
          💾 Lưu tóm tắt (tự động ghép phần mới)
        </button>
        <button onClick={() => { setShowImportModal(false); setImportText(""); }} style={{ flex: 1, padding: "12px", backgroundColor: "#e0e0e0", color: "#333", border: "none", borderRadius: "8px", fontWeight: "bold", cursor: "pointer" }}>
          Hủy
        </button>
      </div>
    </div>
...
function NotebookScreen({ globalStats, onBack, onSaveWord, onRemoveWord, onMoveWord, onMoveManyWords, onRemoveManyWords, onUploadGrammarFile, customGrammarNotes = [], defaultTab = "vocab", currentUser, onRefreshNotes, onUpdateWordObjs }) {  const [activeTab, setActiveTab] = useState(defaultTab);
 ...
 const handleSaveJson = async () => {
  ...
  const fetchAI = async (wordInput, currentTab, retriedAfterModelRefresh = false, temporaryRetryCount = 0) => {
    return retryWithNewKey(async (apiKey) => {
      ...
      let prompt = currentTab === "grammar"
        ...
        ? `Phân tích collocation (cụm từ cố định) tiếng Anh dùng trong TOEIC: "${wordInput}".\nCHỈ TRẢ VỀ DUY NHẤT 1 OBJECT JSON, KHÔNG giải thích thêm:\n{"word": "Collocation đầy đủ (VD: make a decision)", "phonetic": "Phiên âm IPA của từ khóa chính", "meaning": "Nghĩa tiếng Việt TỐI ĐA 6 TỪ", "usage": "1 câu ví dụ ngắn trong ngữ cảnh TOEIC", "synonym": "2-4 collocation tương đương hoặc từ đồng nghĩa", "topic": "Chủ đề/lĩnh vực 1-3 từ tiếng Việt (VD: Công việc, Tài chính...)"}`
        : `Phân tích từ/cụm từ tiếng Anh: "${wordInput}".\nCHỈ TRẢ VỀ DUY NHẤT 1 OBJECT JSON, KHÔNG giải thích thêm:\n{"word": "Từ chuẩn (kèm loại từ)", "phonetic": "Phiên âm IPA", "noun_meaning": "Nghĩa (n) TỐI ĐA 5 TỪ TIẾNG VIỆT, để trống nếu không có", "verb_meaning": "Nghĩa (v) TỐI ĐA 5 TỪ TIẾNG VIỆT, để trống nếu không có", "adj_meaning": "Nghĩa (adj/adv) TỐI ĐA 5 TỪ TIẾNG VIỆT, để trống nếu không có", "meaning": "Nghĩa chung TỐI ĐA 5 TỪ nếu không chia loại từ được", "synonym": "tối thiểu 3 từ đồng nghĩa và tối đa là 7 từ đồng nghĩa", "usage": "1 câu ví dụ ngắn", "topic": "Chủ đề/lĩnh vực 1-3 từ tiếng Việt (VD: Công việc, Du lịch, Y tế...)"}`;
  ...
  // 🏷️ AI TỰ ĐỘNG PHÂN LOẠI CHỦ ĐỀ (chỉ hỏi topic, không hỏi lại nghĩa -> nhanh & tiết kiệm quota hơn)
  const fetchTopicsBatch = async (wordsArray, existingTopics = []) => {
    return retryWithNewKey(async (apiKey) => {
      const API_KEY = getActiveKey();
      if (!API_KEY) throw new Error("No_API");
      ...
      if (!window.globalCachedModel) {
        ...
        if (listData.error) {
          ...
          if (msg.includes("quota") || msg.includes("expired") || listData.error.code === 429 || listData.error.code === 400) {
            const hasNextKey = markKeyExhausted();
            if (hasNextKey) { await new Promise(r => setTimeout(r, 1500)); return fetchTopicsBatch(wordsArray, existingTopics); }
          }
          throw new Error(listData.error.message);
        }
        const textModels = (listData.models || []).filter(m => m.supportedGenerationMethods && m.supportedGenerationMethods.includes("generateContent"));
      ...
      const suggestHint = existingTopics.length > 0
        ? `Ưu tiên dùng lại các chủ đề đã có nếu phù hợp: ${existingTopics.join(", ")}. Chỉ tạo chủ đề mới nếu không từ nào trong danh sách trên phù hợp.`
        : "";
      const prompt = `Phân loại CHỦ ĐỀ/LĨNH VỰC cho từng từ vựng tiếng Anh TOEIC sau đây, mỗi chủ đề chỉ 1-3 từ tiếng Việt ngắn gọn (VD: Công việc, Du lịch, Y tế, Tài chính, Công nghệ, Gia đình, Ẩm thực, Giáo dục, Mua sắm, Thời tiết...). ${suggestHint}\nDanh sách từ: ${wordsArray.join(", ")}\nCHỈ TRẢ VỀ DUY NHẤT 1 MẢNG JSON, đúng thứ tự, đủ số lượng từ đã cho, KHÔNG giải thích thêm:\n[{"word": "từ gốc y hệt trong danh sách", "topic": "Chủ đề"}]`;
...
const handleSaveToFile = async () => {
  ...
  const startManualEdit = () => {
    ...
    setManualInputs({
        ...
        topic: wordDetailModal.detail?.topic || ""
    });
  ...
  const saveManualEdit = () => {
      ...
      playSound("click");
      // Nếu đã nhập riêng (n)/(v)/(adj) thì không cần Nghĩa chung nữa (tránh trùng lặp khi ôn tập)
      const hasSplitMeaning = manualInputs.noun_meaning.trim() || manualInputs.verb_meaning.trim() || manualInputs.adj_meaning.trim();
      const updatedWordObj = {
          word: wordDetailModal.wordStr, 
          phonetic: manualInputs.phonetic.trim(),
          meaning: hasSplitMeaning ? "" : manualInputs.meaning.trim(),
          noun_meaning: manualInputs.noun_meaning.trim(),
          verb_meaning: manualInputs.verb_meaning.trim(),
          adj_meaning: manualInputs.adj_meaning.trim(),
          usage: manualInputs.usage.trim(),
          synonym: manualInputs.synonym.trim(),
          structure: manualInputs.structure.trim(),
          topic: (manualInputs.topic || "").trim()
      };
...
${wordsMissingFields.map((entry, index) => `${index + 1}. ${entry.word} (thiếu: ${entry.missingFields.join(", ")})`).join("\n")}`;
        ...
        {activeTab !== "grammar" && (
          <button
            disabled={isTagging || isReloading || isAdding}
            onClick={async () => {
              const stats2 = globalStats[activeTab] || {};
              const dict2 = stats2.addedWordsObj || [];
              const normalizeW2 = (w) => (typeof w === "string" ? w : (w?.word || "")).toLowerCase().replace(/\s*\(.*?\)\s*/g, "").trim();

              const allWords2 = [
                ...(stats2.savedWords || []),
                ...(stats2.masteredWords || []),
                ...(stats2.wrongWords || []),
              ];
              const seen2 = new Set();
              const wordsMissingTopic = [];
              for (const w of allWords2) {
                const wStr = normalizeW2(w);
                if (!wStr || seen2.has(wStr)) continue;
                seen2.add(wStr);
                const existing = dict2.find(obj => normalizeW2(obj) === wStr);
                if (!existing?.topic?.trim()) {
                  wordsMissingTopic.push(typeof w === "string" ? w : (w?.word || wStr));
                }
              }

              if (wordsMissingTopic.length === 0) return alert("✅ Tất cả từ đã có chủ đề rồi!");
              if (!window.confirm(`Tìm thấy ${wordsMissingTopic.length} từ chưa có chủ đề.\nCho AI tự động phân loại chủ đề cho các từ này?`)) return;

              setIsTagging(true);
              setTagProgress({ done: 0, total: wordsMissingTopic.length });

              const existingTopicsList = [...new Set(dict2.map(o => o.topic).filter(Boolean))];
              let updatedObjs2 = [...dict2];
              const BATCH_SIZE = 15;

              for (let i = 0; i < wordsMissingTopic.length; i += BATCH_SIZE) {
                const batch = wordsMissingTopic.slice(i, i + BATCH_SIZE);
                try {
                  const results = await fetchTopicsBatch(batch, existingTopicsList);
                  results.forEach(r => {
                    if (!r?.word || !r?.topic) return;
                    const normStr = normalizeW2(r.word);
                    const idx = updatedObjs2.findIndex(obj => normalizeW2(obj) === normStr);
                    if (idx !== -1) {
                      updatedObjs2[idx] = { ...updatedObjs2[idx], topic: r.topic.trim() };
                    } else {
                      updatedObjs2.push({ word: batch.find(b => normalizeW2(b) === normStr) || r.word, topic: r.topic.trim() });
                    }
                    if (!existingTopicsList.includes(r.topic.trim())) existingTopicsList.push(r.topic.trim());
                  });
                } catch (e) {
                  console.warn("Lỗi phân loại chủ đề batch:", batch, e);
                }
                setTagProgress({ done: Math.min(i + BATCH_SIZE, wordsMissingTopic.length), total: wordsMissingTopic.length });
                await new Promise(r => setTimeout(r, 300));
              }

              if (onUpdateWordObjs) {
                await onUpdateWordObjs(activeTab, updatedObjs2);
              } else {
                await onSaveWord(activeTab, updatedObjs2);
              }
              setIsTagging(false);
              alert(`✅ Đã tự động gắn chủ đề cho ${wordsMissingTopic.length} từ!`);
            }}
            style={{ background:"rgba(255,255,255,0.18)", border:"none", color:"white", borderRadius:"10px", padding:"6px 14px", cursor: isTagging ? "not-allowed":"pointer", fontWeight:"bold", fontSize:"13px", fontFamily:"inherit", opacity: isTagging ? 0.6 : 1, marginLeft: "6px" }}>
     ...
     {wordDetailModal && (() => {
        ...
        <div onClick={closeDetailModal} style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.7)", zIndex: 1100, display: "flex", justifyContent: "center", alignItems: "center", padding: "20px", boxSizing: "border-box", cursor: "pointer" }}>
            ...
            <div style={{ backgroundColor: "white", borderRadius: "16px", padding: "22px 25px 25px", textAlign: "center", animation: "popIn 0.3s", boxShadow: "0 10px 30px rgba(0,0,0,0.3)", cursor: "default", height: "480px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
                ...
                <h2 style={{ fontSize: "26px", color: "#2196F3", margin: "0 0 5px 0", display: "flex", alignItems: "center", justifyContent: "center", gap: "10px", flexWrap: "wrap" }}>
                  {wordDetailModal.wordStr}
                  {activeTab !== "grammar" && wordDetailModal.detail?.topic && (
                    <span style={{ fontSize: "13px", fontWeight: "bold", color: "#8e24aa", backgroundColor: "#f3e5f5", padding: "3px 10px", borderRadius: "20px", border: "1px solid #ce93d8" }}>
                      🏷️ {wordDetailModal.detail.topic}
                    </span>
                  )}
                </h2>
                {!isEditingManual && (
                ...
                {isEditingManual && (
                    <div style={{ marginTop: "15px", textAlign: "left" }}>
                        ...
                        {activeTab === "grammar" ? (
                          ...
                          <>
                            <label style={{ fontSize: "12px", color: "#666", fontWeight: "bold" }}>📗 Nghĩa (n) — danh từ:</label>
                            <input type="text" value={manualInputs.noun_meaning} onChange={(e) => setManualInputs({...manualInputs, noun_meaning: e.target.value})} placeholder="VD: quyển sách" style={editInputStyle}/>

                            <label style={{ fontSize: "12px", color: "#666", fontWeight: "bold" }}>📘 Nghĩa (v) — động từ:</label>
                            <input type="text" value={manualInputs.verb_meaning} onChange={(e) => setManualInputs({...manualInputs, verb_meaning: e.target.value})} placeholder="VD: đặt chỗ trước" style={editInputStyle}/>

                            <label style={{ fontSize: "12px", color: "#666", fontWeight: "bold" }}>📙 Nghĩa (adj/adv) — tính từ/trạng từ:</label>
                            <input type="text" value={manualInputs.adj_meaning} onChange={(e) => setManualInputs({...manualInputs, adj_meaning: e.target.value})} placeholder="VD: đầy đủ" style={editInputStyle}/>

                            <label style={{ fontSize: "12px", color: "#666", fontWeight: "bold" }}>🔖 Nghĩa chung (chỉ dùng nếu từ không chia được loại từ trên):</label>
                            <input type="text" value={manualInputs.meaning} onChange={(e) => setManualInputs({...manualInputs, meaning: e.target.value})} placeholder="Định nghĩa ngắn gọn..." style={editInputStyle}/>
                          </>
                        )}
                        ...
                        {activeTab !== "grammar" && (
                          <>
                            <label style={{ fontSize: "12px", color: "#666", fontWeight: "bold" }}>🏷️ Chủ đề (giúp bạn nhớ từ theo nhóm, VD: Công việc, Du lịch, Y tế...):</label>
                            <input type="text" value={manualInputs.topic || ""} onChange={(e) => setManualInputs({...manualInputs, topic: e.target.value})} placeholder="VD: Công việc" style={editInputStyle} list="topic-suggestions"/>
                            <datalist id="topic-suggestions">
                              {[...new Set((globalStats[activeTab]?.addedWordsObj || []).map(w => w.topic).filter(Boolean))].map(t => (
...
function App() {
  ...
  useEffect(() => {
    const rawImport = new URLSearchParams(window.location.search).get("importWord");
    if (!rawImport) return;

    try {
      const importedWord = JSON.parse(rawImport);
      if (importedWord?.word) setExternalImport(importedWord);
    } catch (error) {
      console.error("Không đọc được từ gửi từ app Dictionary:", error);
    }
...
return (
  ...
  >
    ...
    {externalImport && (
      <div onClick={() => setExternalImport(null)} style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.65)", zIndex: 3000, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
        <div onClick={(event) => event.stopPropagation()} style={{ width: "100%", maxWidth: "420px", background: "white", borderRadius: "16px", padding: "24px", boxShadow: "0 16px 40px rgba(0,0,0,0.3)" }}>
          <div style={{ fontSize: "13px", color: "#607d8b", fontWeight: "700", marginBottom: "8px" }}>TỪ TỪ APP DICTIONARY</div>
          <h2 style={{ margin: "0 0 4px", color: "#1565c0", fontSize: "26px" }}>{externalImport.word}</h2>
          {externalImport.partOfSpeech && <div style={{ display: "inline-block", color: "#283593", background: "#e8eaf6", borderRadius: "12px", padding: "3px 9px", fontSize: "12px", fontWeight: "700", marginBottom: "14px" }}>{externalImport.partOfSpeech}</div>}
          <div style={{ background: "#f5f9ff", border: "1px solid #d7e8ff", borderRadius: "10px", padding: "13px", color: "#37474f" }}>
            {externalImport.phonetic && <div style={{ fontStyle: "italic", color: "#78909c", marginBottom: "6px" }}>{externalImport.phonetic}</div>}
            <div style={{ fontWeight: "700", color: "#2e7d32", marginBottom: "6px" }}>{externalImport.meaningVi || externalImport.meaning || "Chưa có nghĩa tiếng Việt"}</div>
            {externalImport.meaningEn && <div style={{ fontSize: "13px", fontStyle: "italic", color: "#607d8b" }}>{externalImport.meaningEn}</div>}
            {externalImport.examples?.[0] && <div style={{ marginTop: "10px", fontSize: "13px" }}>“{externalImport.examples[0]}”</div>}
          </div>
          <div style={{ display: "flex", gap: "10px", marginTop: "18px" }}>
            <button onClick={() => setExternalImport(null)} style={{ flex: 1, padding: "11px", border: "none", borderRadius: "9px", background: "#eceff1", color: "#455a64", cursor: "pointer", fontWeight: "700" }}>Bỏ qua</button>
            <button onClick={async () => {
              const wordData = {
                ...externalImport,
                meaning: externalImport.meaning || externalImport.meaningVi || "",
                usage: externalImport.usage || externalImport.examples?.[0] || "",
                synonyms: Array.isArray(externalImport.synonyms) ? externalImport.synonyms.join(", ") : (externalImport.synonyms || ""),
              };
              await handleSaveDifficultWord("vocab", wordData);
              setExternalImport(null);
              setScreen("notebook");
            }} style={{ flex: 1, padding: "11px", border: "none", borderRadius: "9px", background: "#ff9800", color: "white", cursor: "pointer", fontWeight: "700" }}>🔖 Lưu vào Sổ Tay</button>
          </div>
        </div>
      </div>
    )}
